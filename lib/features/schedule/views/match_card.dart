...
class MatchCard extends StatefulWidget {
  const MatchCard({Key? key, required this.gameCode, required this.matchDetails}) : super(key: key);
...
class _MatchCardState extends State<MatchCard> {
  ...
  Widget build(BuildContext context) {
    ...
    Color statusColor = widget.matchDetails.status == 'finished'
        ...
        : (widget.matchDetails.status == 'canceled' || widget.matchDetails.status == 'postponed'
            ? kPastColor //Colors.grey
            : Colors.white);
    var matchStartTime = DateTime.fromMillisecondsSinceEpoch(int.parse(widget.matchDetails.startTimestamp!) * 1000).toString().substring(11, 16);
    return InkWell(
      onTap: () async {
        ...
        child: Container(
          ...
          child: Row(
            children: [
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          if (widget.matchDetails.status == 'canceled' || widget.matchDetails.status == 'postponed')
                            const Text('')
                          else if (widget.matchDetails.status != 'notstarted')
                            Text(
                              widget.matchDetails.homeScore.toString() != 'null' ? widget.matchDetails.homeScore.toString() : '0',
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          if (widget.matchDetails.status == 'canceled' || widget.matchDetails.status == 'postponed')
                            ...
                            Text(
                              widget.matchDetails.awayScore.toString() != 'null' ? widget.matchDetails.awayScore.toString() : '0',
                              style: GoogleFonts.robotoMono(
                                textStyle: TextStyle(
                                  fontSize: SizeConfig.safeBlockHorizontal! * 4,
                                  color: statusColor,
                ...
                SizedBox(
                  ...
                  child: Center(
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: kLiveColor, borderRadius: BorderRadius.circular(SizeConfig.safeBlockHorizontal! * 1)),
                    ),
                  ),
                )
              ...
              else if (DateTime.fromMillisecondsSinceEpoch(int.parse(widget.matchDetails.startTimestamp!) * 1000)
                  .isAfter(DateTime.now())) //widget.matchDetails.status == 'notstarted')
                FutureBuilder(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (isNotified == true) {
                      return InkWell(
                        onTap: () {
                          /// remove notification
                          NotificationsHelper.cancel(int.parse(widget.matchDetails.id!));
                      ...
                      return InkWell(
                        onTap: () {
                          ...
                          NotificationsHelper.showScheduledNotifications(
                            servingTime: DateTime.fromMillisecondsSinceEpoch(int.parse(widget.matchDetails.startTimestamp!) * 1000).subtract(Duration(minutes: settingsController.minutes.value)),
                            id: int.parse(widget.matchDetails.id!),
                            title: 'Match Time',
                            body: '${widget.matchDetails.homeTeam} vs ${widget.matchDetails.awayTeam} is about to start.', payload: widget.matchDetails.id.toString(),
                          );
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                             SnackBar(
                              content: Obx(()=>Text('You will be notified ${settingsController.minutes.value} minutes before the match.')),
                            ),
                          );
                        },
                        child: SizedBox(
